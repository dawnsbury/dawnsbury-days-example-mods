...
public static class PortalistClassLoader
{
    ...
    private static IEnumerable<Feat> CreateFeats()
    {
        yield return new ClassSelectionFeat(ModManager.RegisterFeatName("FeatPortalist", "Portalist"),
                ...
                @"{b}1. Portal and portal points.{/b} You gain a daily reservoir of 'portal points' equal to your level plus your intelligence modifier. You can spend an action and expend a portal point to teleport up to your speed to a square you can see. This is a flourish action that doesn't provoke attacks of opportunity.

{b}2. Quick.{/b} You gain a +1 status bonus to initiative rolls, if you're wearing no armor or light armor only.

{b}3. Post-combat recovery.{/b} At the end of each encounter, if you spent at least one portal point in that encounter, you regain one portal point.

{b}4. Portalist feat.{/b} You choose and get a portalist feat. Portalist feats often allow you to create more powerful or specialized portals instead of your standard portal. 
...
{b}Level 4:{/b} Portalist feat", null)
            ...
            .WithOnCreature(creature =>
            {
                creature.AddQEffect(new QEffect()
                {
                    ...
                    {
                        return Wrap(CreateNormalPortal(qf, IllPortal, "Standard Portal", "Expend 1 portal point to teleport to a square you can see within range.")
        ...
        yield return new TrueFeat(ModManager.RegisterFeatName("Ally Portal"), 1, "You pull your friend alongside you.",
                "Choose an adjacent ally, then you teleport as normal, then you pull your ally through the portal to an adjacent square.", [TPortalist, Trait.Flourish])
            .WithActionCost(1)
            .WithIllustration(IllPortal)
            .WithPermanentQEffect("You can bring an ally with you through a portal.", qf => qf.Id = QAllyPortal);
        yield return new TrueFeat(ModManager.RegisterFeatName("Boomerang Portal"), 1, "You swiftly move in and out of combat.",
            ...
            .WithPermanentQEffect("You can teleport, make a melee Strike, then teleport back.", qf =>
            {
                ...
                {
                    var action = CreateNormalPortal(qff, IllustrationName.AerialBoomerang256, "Boomerang Portal", "Expend 1 portal point to teleport to a square within range, then make a melee Strike, then teleport back.")
                        .WithEffectOnChosenTargets(async (spell, caster, targets) =>
                        {
        ...
        yield return new TrueFeat(ModManager.RegisterFeatName("Attack Portal"), 1, "You come out of the portal with your weapons imbued with extradimensional energy.",
            ...
            .WithPermanentQEffect("Teleport, then you gain a +2 status bonus to your next attack this turn.", qf =>
            {
                ...
                qf.ProvideMainAction = qff =>
                {
                    var action = CreateNormalPortal(qff, new SideBySideIllustration(IllPortal, IllustrationName.Swords), "Attack Portal", "Expend 1 portal point to teleport to a square within range and gain a +2 status bonus to your next attack this turn.")
                        .WithEffectOnChosenTargets(async (spell, caster, targets) =>
                        {
                            caster.PersistentUsedUpResources.UsedUpActions.Add(SUsedPoint);
                            PortalistTeleport(caster, targets.ChosenTile!);
        ...
        yield return new TrueFeat(ModManager.RegisterFeatName("Covering Portal"), 1, "You summon a portal in such a way that it serves as cover from attacks.",
            ...
            .WithActionCost(1)
            .WithIllustration(IllustrationName.TakeCover)
            .WithPermanentQEffect("You may choose to teleport as normal. Whether or not you teleport, you gain a +2 circumstance bonus to your AC until you move or until the beginning of your next turn.", qf => qf.Id = QCoveringPortal);
        yield return new TrueFeat(ModManager.RegisterFeatName("Double-Hop Portal"), 1, "You create two portals, jump into the first one, then out the first one into the second.",
            ...
            .WithPermanentQEffect("Teleport as normal. Then do it again.", qf => qf.Id = QDoubleHopPortal);
        yield return new TrueFeat(ModManager.RegisterFeatName("Elemental Blast Portal"), 2, "You cause the extradimensional matter of a portal to explode in an unstable vortex of elemental energy.",
            ...
            .WithPermanentQEffect("You can open a portal explosively to deal elemental damage.", qf => qf.Id = QElementalBlastPortal);
        yield return new TrueFeat(ModManager.RegisterFeatName("Shielding Portal"), 2, "You prepare to summon a portal at a moment's notice that would shunt away incoming projectiles.",
            ...
            .WithPermanentQEffect("You can prepare a reaction to deflect an incoming projectile.", qf => qf.Id = QShieldingPortal);
        yield return new TrueFeat(ModManager.RegisterFeatName("Summoning Portal"), 4, "You don't cross your portal and instead use it to call in creatures of energy.",
            ...
            .WithPermanentQEffect("You can summon elementals as per the {i}summon elemental{/i} spell.", qf => qf.Id = QSummoningPortal);
    }
    ...
    {   
        ...
        return new CombatAction(portalist, illustration, name + " [" + pointsLeft + " points left]", [TPortalist, Trait.Move, Trait.Flourish], description, target)
    ...
    private static Possibility Wrap(CombatAction portalAction)
    {
        ...
        return new ActionPossibility(portalAction).WithPossibilityGroup("Create a Portal [" + pointsLeft + " points]");
    }
